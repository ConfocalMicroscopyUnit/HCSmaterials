...
rawParameters=File.openAsString(getDirectory("macros")+"HCS-Check-Results-parameters.txt");
...
else if(nSlices==1 && nChannels>1)//a collection of single .tif images from different channels were captured per field
{
	...
	{
		//if(matches(imagesNamesArray[i], imageToCheckPath)
		//print("imagesNamesArray["+i+"]: "+imagesNamesArray[i]);
